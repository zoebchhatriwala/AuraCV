...
export default function Editor() {
  ...
  const { fetchResume, currentResume, updateResume, settings, updateSettings } =
    useAppStore();
  ...
  const [previewMode, setPreviewMode] = useState<
    "split" | "full-editor" | "full-preview"
  >("split");
  const [mobileTab, setMobileTab] = useState<"editor" | "preview" | "copilot">(
    "editor",
  );
  ...
  const [copilotInitialContext, setCopilotInitialContext] = useState<
  ...
  const handleAddSection = async (type: string, title: string) => {
    ...
    const maxPos = Math.max(
      -1,
      ...currentResume.sections.map((s) => s.position),
    );
  ...
  const handleToggleVisibility = async (section: Section) => {
    ...
    await resumeApi.updateSection(id, section.id, {
      ...section,
      is_visible: isCurrentlyVisible ? 0 : 1,
    });
    await fetchResume(id);
    refreshPreview();
  };
  ...
  const handleMoveSection = async (
    ...
    if (!id || !currentResume) return;
    const sorted = currentResume.sections
      .slice()
  ...
  const handleOpenCopilotWithBullet = (
    bullet: string,
    context: { sectionId: string; index: number },
  ) => {
  ...
  const handleApplyBulletFromCopilot = async (
    ...
    const nextContent = JSON.parse(JSON.stringify(sec.content || [])) as Array<
    ...
    if (typeof bulletIndex === "number" && nextContent.length > 0) {
      for (const entry of nextContent) {
        if (
          ...
          entry.bullets[bulletIndex] !== undefined
        ) {
    ...
    if (!replaced && originalBullet) {
      for (const entry of nextContent) {
        if (entry.bullets && Array.isArray(entry.bullets)) {
          const bIdx = entry.bullets.findIndex(
            (b: string) => b === originalBullet,
          );
          if (bIdx !== -1) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      >
        ...
        <div
          className="flex md:hidden items-center justify-between gap-2 w-full pt-1 border-t"
          style={{ borderColor: "var(--border-subtle)" }}
        >
          <div
        ...
        <div className="hidden md:flex items-center gap-2.5">
          ...
          >
            {(["split", "full-editor", "full-preview"] as const).map((mode) => (
              <button
                ...
                style={{
                  color:
                    previewMode === mode ? undefined : "var(--text-secondary)",
                }}
              >
                {mode === "split"
                  ? "Split View"
          ...
          >
            ...
            <span>Copilot</span>
            <span
              className={`hidden sm:inline-block text-[10px] px-1 py-0.2 rounded border font-mono ${
                showCopilot
          ...
          <div className="p-4 sm:p-5 space-y-3 max-w-3xl mx-auto w-full">
            {(() => {
              const sortedSections = currentResume.sections
                .slice()
                .sort((a, b) => a.position - b.position);
              const headerIndex = sortedSections.findIndex(
              ...
              return sortedSections.map((section, idx, arr) => {
                ...
                const isFirst =
                  isHeader || (headerIndex === 0 ? idx <= 1 : idx === 0);
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span
                  className="px-1.5 sm:px-2 text-[11px] font-medium"
                  style={{ color: "var(--text-secondary)" }}
                >
                  {zoomLevel}%
                </span>
    ...
    >
      ...
      >
        ...
        <button
          ...
          title={
            isVisible
              ? "Visible on resume (Click to hide)"
              : "Hidden from resume (Click to show)"
          }
        >
          {isVisible ? (
